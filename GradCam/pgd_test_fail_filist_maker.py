...
fo = open("./pgd_fail_test.flist", "w")

for label in range(10):
    root = '../../fin_dataset/cifar10/test/pgd'


    ori_picture = sorted(glob.glob(os.path.join(root, 'fail/' + str(label)) + '/*.*'))
    ...
    num = 0
    for i in range(max(ori_picture_num, 100)):
        now_i = random.randint(0, ori_picture_num - 1)
        for j in range(10):
            s = "{:04d}".format(now_i)
            sj = "{:04d}".format(j)
            path = '../../../fin_dataset/cifar10/test/pgd/fail/' + str(label) + '/' + s +'.png' + ',' + '../../../fin_dataset/cifar10/test/pgd/fail/' + str(label) + '/' + s  +'.png' + ',' + sj + '.png'
            fo.write(path + '\n')
            # flist.append(path)
