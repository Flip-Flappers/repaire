...
if attack == 'BIM':
    ...
    if target:
        ...
        adversary = PGDAttack(
            ...
            loss_fn=nn.CrossEntropyLoss(reduction="sum"),
            eps=2 / 255,
            nb_iter=10, eps_iter=2 / 255 / 5, clip_min=0, clip_max=1.0,
            targeted=False)
    # FGSM
elif attack == 'FGSM':
    ...
    adversary = CarliniWagnerL2Attack(
        ...
        max_iterations=12,
        targeted=target, clip_min=0, clip_max=1.0)
...
for data in tqdm(trainloader):
    ...
    if fin_label != true_label:
        ...
        fgsm_image.save(root + 'cw/success/' + str(int(true_label)) + '/' + s + '.png')
        num[true_label] += 1
    else:
        s = "{:04d}".format(num2[true_label])
        fgsm_image.save(root + 'cw/fail/' + str(int(true_label)) + '/' + s + '.png')
        num2[true_label] += 1