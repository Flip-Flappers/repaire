...
class SiLU(nns.Module):
    def forward(self, x):
        return x * torch.sigmoid(x)

class EmbedBlock(nns.Module):
    """
...
class EmbedSequential(nns.Sequential, EmbedBlock):
    """
...
class Upsample(nns.Module):
    ...
    def __init__(self, channels, use_conv, out_channel=None):
        ...
        if use_conv:
            self.conv = nns.Conv2d(self.channels, self.out_channel, 3, padding=1)
...
class Downsample(nns.Module):
    ...
    def __init__(self, channels, use_conv, out_channel=None):
        ...
        if use_conv:
            ...
            self.op = nns.AvgPool2d(kernel_size=stride, stride=stride)
...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.in_layers = nns.Sequential(
            normalization(channels),
            SiLU(),
            nns.Conv2d(channels, self.out_channel, 3, padding=1),
        )
        ...
        if up:
            ...
            self.h_upd = self.x_upd = nns.Identity()

        self.emb_layers = nns.Sequential(
            SiLU(),
            nns.Linear(
                emb_channels,
        ...
        self.out_layers = nns.Sequential(
            normalization(self.out_channel),
            SiLU(),
            nns.Dropout(p=dropout),
            zero_module(
                nns.Conv2d(self.out_channel, self.out_channel, 3, padding=1)
            ),
        )

        if self.out_channel == channels:
            self.skip_connection = nns.Identity()
        elif use_conv:
            self.skip_connection = nns.Conv2d(
                channels, self.out_channel, 3, padding=1
            )
        else:
            self.skip_connection = nns.Conv2d(channels, self.out_channel, 1)
...
class AttentionBlock(nns.Module):
    """
    ...
    def __init__(
        ...
        self.qkv = nns.Conv1d(channels, channels * 3, 1)
        ...
        self.proj_out = zero_module(nns.Conv1d(channels, channels, 1))
...
class QKVAttentionLegacy(nns.Module):
    """
...
class UNet(nns.Module):
    ...
    def __init__(
        ...
        self.cond_embed = nns.Sequential(
            nns.Linear(inner_channel, cond_embed_dim),
            SiLU(),
            nns.Linear(cond_embed_dim, cond_embed_dim),
        )

        ch = input_ch = int(channel_mults[0] * inner_channel)
        self.input_blocks = nns.ModuleList(
            [EmbedSequential(nns.Conv2d(in_channel, ch, 3, padding=1))]
        )
        ...
        self.output_blocks = nns.ModuleList([])
        ...
        self.out = nns.Sequential(
            normalization(ch),
            SiLU(),
            zero_module(nns.Conv2d(input_ch, out_channel, 3, padding=1)),
        )
...
if __name__ == '__main__':
    ...
    emb = torch.ones((b, ))
    out = model(x, emb)
    h_draw.build_graph(model, [x, emb]);
    vis_graph = h_draw.build_graph(model, [x , emb]))  # 获取绘制图像的对象
